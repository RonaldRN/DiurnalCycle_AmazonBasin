...
# File: mean_seasonal_atmospheric_circulation_500hPa.py 
#
# Descriptiopn:
# Esse script faz os gráficos da circulação atmosférica sazonal média 
# no nível de 500 hPa (vetores), incluindo a topografia dos Anes (acima do nível de 1500 msnm),
...
ds = xr.open_dataset('/home/ronaldrn/ronald/mestrado/NETCDF_ATMOSPHERIC_CIRCULATION/circulation_mean_season_BA.nc')
...
title = ['a) Circulação atmosférica a 500 hPa\nPeríodo:DJF [2001-2020]', 'c) Circulação atmosférica a 500 hPa\nPeríodo:JJA [2001-2020]',\
        'b) Circulação atmosférica a 500 hPa\nPeríodo:MAM [2001-2020]', 'd) Circulação atmosférica a 500 hPa\nPeríodo:SON [2001-2020]']
...
for i in range(1,5,1):
    ...
    strm = ax[i-1].streamplot(ds_mean.longitude, ds_mean.latitude, ds_mean['u'].isel(season=i-1, level=3).values,\
        ds_mean['v'].isel(season=i-1, level=3).values, color='black', transform = ccrs.PlateCarree(), density=[2,2])

print('11. Terminou de executar o loop')
#
# ------------------------------------------------------------------------------------------------------------------------------
#
print('12. Configurar o colorbar da elevação do terreno')
#Adicionando a barra de cor
#cbar = fig.colorbar(im, ax=axes[:,1], ticks=np.arange(0, ds_topo_ba.z.max(), 200),shrink=0.5, pad=0.05, fraction=.1)
#legenda da colorbar 
#cbar.set_label('Elevação do terreno (m)',size=16)
...
fig.savefig('/home/ronaldrn/ronald/mestrado/PLOTS_CIRCULATION/mean_sazonal_circulation_500hPa.png', format='png', dpi = 300)
