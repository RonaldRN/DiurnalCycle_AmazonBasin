...
# File: mean_seasonal_atmospheric_circulation_850hPa.py 
#
# Descriptiopn:
# Esse script faz os gráficos da circulação atmosférica sazonal média 
# no nível de 850 hPa (vetores), incluindo a topografia dos Anes (acima do nível de 1500 msnm),
...
title = ['a) Circulação atmosférica a 850 hPa\nPeríodo:DJF [2001-2020]', 'c) Circulação atmosférica a 850 hPa\nPeríodo:JJA [2001-2020]',\
        'b) Circulação atmosférica a 850 hPa\nPeríodo:MAM [2001-2020]', 'd) Circulação atmosférica a 850 hPa\nPeríodo:SON [2001-2020]']
...
for i in range(1,5,1):
    ...
    vetor = ax[i-1].quiver(ds_mean.longitude, ds_mean.latitude, ds_mean['u'].isel(season=i-1, level=4).values,\
        ds_mean['v'].isel(season=i-1, level=4).values, width=0.004, headlength=6, transform= ccrs.PlateCarree(), regrid_shape=30, color='black')
...
fig.savefig('/home/ronaldrn/ronald/mestrado/PLOTS_CIRCULATION/mean_sazonal_circulation_850hPa.png', format='png', dpi = 300)
