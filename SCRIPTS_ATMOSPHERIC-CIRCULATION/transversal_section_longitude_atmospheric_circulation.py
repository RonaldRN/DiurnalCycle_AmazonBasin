...
# File: transversal_section_longitude_atmospheric_circulation.py 
#
# Descriptiopn:
# Esse script faz os gráficos dos cortes transversais para uma longitude constante da circulação atmosférica 
# utilizando as componentes zonal e meridional do vento (u,v), a velocidade vertical omega (w) e a umidade
# específica incluindo a topografia dos Anes. Os períodos eleitos são os meses de Janeiro, Abril, Julho e Outubro
# Nos horários de máximas taxas de precipitação.
# Os plots são baseados no arquivo de Segura et al. (2019)
#
"""
#
# -------------------------------------------------------------------------------------------------------------------
...
ds_topog = xr.open_dataset('/home/ronaldrn/ronald/mestrado/NETCDF_ATMOSPHERIC_CIRCULATION/geopotential_levels.nc')
ds_topog['z'].sel(latitude = slice(5,-20), longitude = -70)
...
if periodo == "Janeiro":
    ...
    ds = ds_mean_january.sel(latitude = slice(5,-20), longitude = -70)
    # Title
    ...
    # Fazer um corte sobre a região de interesse
    ds = ds_mean_april.sel(latitude = slice(5,-20), longitude = -70)
    # Title
    ...
    # Fazer um corte sobre a região de interesse
    ds = ds_mean_july.sel(latitude = slice(5,-20), longitude = -70)
    # Title
    ...
    # Fazer um corte sobre a região de interesse
    ds = ds_mean_october.sel(latitude = slice(5,-20), longitude = -70)
    # Title
...
q = ax[0].contourf(ds.latitude, ds.level, (ds['q']*1000).isel(hour=hour[0]), \
    levels= (colorb), vmax=18, extend='max', cmap='BrBG')

# Plot do vento zonal (m/s) e vertical (cm/s) (vetores)
vetor = ax[0].quiver(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), \
    ds['w'].isel(hour=hour[0])*-10, color='black', width=0.0025, scale=150,\
    headlength=4, headwidth=3, minlength=0.05, minshaft=0.15)

# Vento zonal = 0 m/s
u0 = ax[0].contour(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), colors=['black'], levels=np.arange(0,1,1))
# Vento zonal positivo (vento de oeste)
colorup = np.arange(1,10,1)
up = ax[0].contour(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), colors=['blue'], levels=colorup)
# Agregando os labels do vento zonal positivo
ax[0].clabel(
    up,                        # Agregamos a variável com os contornos
    colors = ['blue'],       # Cor dos labels dos valores dos contornos
...
# Vento zonal negativo (vento de leste)
colorun = np.arange(-10,0,1)
un = ax[0].contour(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), colors=['red'], levels=colorun)
# Agregando os labels do vento zonal negativo
ax[0].clabel(
    un,                        # Agregamos a variável com os contornos
    colors = ['red'],       # Cor dos labels dos valores dos contornos
    manual = False,           # Pôr os labels de forma automática frente à forma manual
    inline = True,            # Valores dentro da linha <- ----15------
    fmt = ' {:.0f}'.format,   # Labels como valores inteiros
    )
...
ax[0].xaxis.set_major_formatter(LatitudeFormatter())
...
ax1 = ax[0].twinx()
# Plot da topografia
topo_line = ds_ba['z'].sel(lon=-70.66, method='nearest').plot(color='gray')
topografia = ds_ba['z'].sel(lon=-70.66, method='nearest')
plt.fill_between(ds_ba.lat, topografia, color='gray', alpha=0.8)
...
q = ax[1].contourf(ds.latitude, ds.level, (ds['q']*1000).isel(hour=hour[1]), \
    levels= (colorb), vmax=18, extend='max', cmap='BrBG')

# Plot do vento zonal e vertical (vetores)
vetor = ax[1].quiver(ds.latitude, ds.level, (ds['v']).isel(hour=hour[1]), \
    ds['w'].isel(hour=hour[1])*-10, color='black', width=0.0025, scale=150,\
    headlength=4, headwidth=3, minlength=0.05, minshaft=0.15)

# Vento zonal = 0 m/s
u0 = ax[1].contour(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), colors=['black'], levels=np.arange(0,1,1))
# Vento zonal positivo (vento de oeste)
colorup = np.arange(1,10,1)
up = ax[1].contour(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), colors=['blue'], levels=colorup)
# Agregando os labels do vento zonal positivo
ax[1].clabel(
    up,                        # Agregamos a variável com os contornos
    colors = ['blue'],       # Cor dos labels dos valores dos contornos
...
# Vento zonal negativo (vento de leste)
colorun = np.arange(-10,0,1)
un = ax[1].contour(ds.latitude, ds.level, ds['v'].isel(hour=hour[0]), colors=['red'], levels=colorun)
# Agregando os labels do vento zonal negativo
ax[1].clabel(
    un,                        # Agregamos a variável com os contornos
    colors = ['red'],       # Cor dos labels dos valores dos contornos
...
ax[1].xaxis.set_major_formatter(LatitudeFormatter())
...
#if periodo == 'Outubro':
#    cbar = fig.colorbar(q, ax = ax[:], ticks = colorb, fraction=0.05, orientation='horizontal', pad=0.15, aspect=10)
    # Label da paleta de cores da umidade específica
#    cbar.set_label('Specific humidity [g/kg]', size = 14, labelpad=-50)
...
fig.savefig('/home/ronaldrn/ronald/mestrado/PLOTS_CIRCULATION/CT_longitude_' + f'{periodo}.png', format='png', dpi = 300)
