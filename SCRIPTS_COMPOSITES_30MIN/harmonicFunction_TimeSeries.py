...
def harmonico(dados, onlycoef = False, harmonic = True):

    # INPUT
    #
    # - dados: série temporal dos composites horários da precipitação
    #
    # OUTPUT
    #
    # -YH1: Séries do 1º Harmônico
    # -YH2: Séries do 2º Harmônico
    # -C1_24h: Amplitude Normalizada do 1º Harmônico
    # -C2_24h: Amplitude Normalizada do 2º Harmônico
    ...
    if harmonic == True:
        # Calculando as Funções harmônicas 1º e 2º
        # # Função do 1º harmônico
        YH1 = C1 * np.cos((2*np.pi*k1*t / n) - fase1)
        # Função do 1º harmônico
        YH2 = C2 * np.cos((2*np.pi*k2*t / n) - fase2)

        return YH1, YH2

    if onlycoef == True:
        # Retornado os parâmetros harmônicos
