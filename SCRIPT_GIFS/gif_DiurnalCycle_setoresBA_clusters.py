...
# 
# File: gif_DiurnalCycle_setoresBA_clusters.py
...
# e para os períodos de DJF, MAM, JJA e SON, incluindo os pontos onde foram extraidos as séries
# temporais dos clusters 
#
'''
###########################################################################################
...
for i in np.arange(0,48,1):
    ...
    for j in np.arange(0,42,1):
        ax.scatter(lon[j], lat[j], s=150, color= "black", marker=r'${0}$'.format(0+cluster[j]), edgecolor="black", alpha=0.8, zorder=10)

    ax.set_title(f'{periodo}' + '[2001-2020] - ' + 'hora: ' + hour[i] + 'UTC - ' + f'Setor: {setor}', fontsize=18, y=1.02)

    fig.tight_layout

    fig.savefig('/home/ronaldrn/ronald/mestrado/gif_horario/' + f'{setor}/' + f'{periodo}_BA_ciclo_diurno_' + f'{setor}_'+ f'{i}_clusters.png',\
      format='png', dpi = 100)
...
if periodo == 'SON':
    ...
    filenames_DJF = [output + "DJF_BA_ciclo_diurno_"+f"{setor}_"+str(i)+"_clusters.png" for i in range(48)]
    filenames_MAM = [output + "MAM_BA_ciclo_diurno_"+f"{setor}_"+str(i)+"_clusters.png" for i in range(48)]
    filenames_JJA = [output + "JJA_BA_ciclo_diurno_"+f"{setor}_"+str(i)+"_clusters.png" for i in range(48)]
    filenames_SON = [output + "SON_BA_ciclo_diurno_"+f"{setor}_"+str(i)+"_clusters.png" for i in range(48)]

    filenames = filenames_DJF + filenames_MAM + filenames_JJA + filenames_SON

    with imageio.get_writer('/home/ronaldrn/ronald/mestrado/gif_horario/' + f'{setor}/' + f'gif_{setor}_clusters.gif', mode='I', duration = 0.2) as writer:
